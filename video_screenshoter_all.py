#!/usr/bin/env python3
"""
Video Screenshot Extractor - ALL FRAMES
Extracts ALL frames from MP4 videos in the assets folder.

Usage: python3 video_screenshoter_all.py <folder_name>
Example: python3 video_screenshoter_all.py death_becomes_her

Output format: {segundo_em_5_digitos}_{frame}.png
Note: This extracts EVERY frame, which can create thousands of images.
"""
...
def extract_screenshots(video_path, output_folder):
    """
    Extract ALL screenshots from video.
    ...
    print(f"⚠️  Warning: This will create {total_frames} image files!")
    ...
    while True:
        ...
        # Calculate which second this frame belongs to (1-based)
        current_second = int(frame_count / fps) + 1
        # Calculate frame number within that second (1-based)
        frame_in_second = int(frame_count % fps) + 1
        
        # Generate filename: {segundo_em_5_digitos}_{frame}.png
        filename = f"{current_second:05d}_{frame_in_second}.png"
        ...
        if frame_count % 100 == 0 or frame_count < 10:
            print(f"Saved: {filename} (frame {frame_count + 1}/{total_frames})")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Extract ALL frames from MP4 videos",
        epilog="Example: python3 video_screenshoter_all.py death_becomes_her"
    )
    parser.add_argument(
        "folder_name",
    ...
    try:
        ...
        # Extract screenshots (all frames)
