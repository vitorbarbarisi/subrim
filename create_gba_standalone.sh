...
echo "=== GBA ROM Standalone (Dados Embedded) ==="

cd r36s_viewer_gba

cat > create_standalone_gba.py << 'PYTHON_EOF'
#!/usr/bin/env python3
...
python3 create_standalone_gba.py

if [ -f "viewer.gba" ]; then
    echo "✅ Standalone GBA ROM created!"
    echo "📊 Size: $(ls -lh viewer.gba | awk '{print $5}')"
    
    # Show what's embedded
    echo "📋 Embedded content:"
    echo "   - Episode titles: CHAVES, FLIPPER"
    echo "   - Subtitle samples"
    echo "   - Menu interface"
    echo "   - Input handling (A/B buttons)"
    
    cd ..
    echo "🚀 Deploy: sudo ./deploy_gba_direct.sh"
else
    echo "❌ Failed to create standalone ROM"
    cd ..
fi
